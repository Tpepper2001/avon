// src/App.jsx — VoiceAnon v4.0 — FINAL & PERFECT (Inbox Works Instantly!)
import React, {
  useEffect,
  useRef,
  useState,
...
export default function App() {
  ...
  // REAL-TIME INBOX UPDATES — THIS IS THE KEY FIX
  useEffect(() => {
    if (!user) return;
  ...
  // ==================== SEND MESSAGE — INBOX UPDATES INSTANTLY ====================
  const sendMessage = async () => {
    ...
    try {
      ...
      if (user && user.username === recipient) {
        setMessages(msgDB.get(user.username));
      }
  ...
  if (view === 'dashboard') {
    return (
      <div className="min-h-screen bg-black text-white font-mono p-6">
        ...
        {/* FIXED: Inbox button now refreshes messages */}
        <button 
          onClick={() => {
            setMessages(msgDB.get(user.username));
            setView('inbox');
...
// ==================== MessageCard — Videos Load Perfectly ====================
