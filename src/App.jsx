...
export default function App() {
    ...
    try {
      ...
      try {
        ...
        if (SR) {
          ...
          console.info('SpeechRecognition not available.');
        }
      } catch (err) {
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      alert('Browser cannot create video from canvas. Try Chrome/Edge desktop.');
      setProcessing(false);
      return;
    }
    ...
    if (audioElementRef.current && audioElementRef.current.src) {
      try {
        ...
        console.warn('Could not autoplay recorded audio', e);
      }
    } else {
